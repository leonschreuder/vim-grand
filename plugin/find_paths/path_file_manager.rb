...
class PathFileManager
    ...
    def self.purgePathsAlreadyInFile(paths = @paths)
        if File.exists?(ProjectControler::PATH_FILE)
            @appending = true

            linesInFile = IO.readlines(ProjectControler::PATH_FILE)
    ...
    def self.writePathsToFile(paths = @paths)
        File.open(ProjectControler::PATH_FILE, 'a') { |file|
            if @appending
                file.write("\n") # Start on a new line
            end
    ...
    def self.retrievePathsWithPreceidingChar(preceidingChar)
        ...
        if File.file?(ProjectControler::PATH_FILE)
            f = File.open(ProjectControler::PATH_FILE, "r")
            f.each_line do |line|
